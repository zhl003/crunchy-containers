...
source "${QiNGCLOUD_DIR}/bin/postgres/setenv.sh"
...
function initdb_logic() {
    ...
    cp "${QiNGCLOUD_DIR}/conf/postgres/pg_hba.conf" /tmp
...
function fill_conf_file() {
    ...
    cp "${QiNGCLOUD_DIR}/conf/postgres/postgresql.conf.template" /tmp/postgresql.conf
...
function waitforpg() {
    ...
    while true; do
        psql -h $PG_PRIMARY_HOST -p $PG_PRIMARY_PORT -U $PG_PRIMARY_USER $PG_DATABASE -f "${QiNGCLOUD_DIR}/bin/postgres/readiness.sql"
        if [ $? -eq 0 ]; then
            echo_info "The database is ready."
            CONNECTED=true
            break
...
function initialize_replica_pre12() {
    ...
    cp "${QiNGCLOUD_DIR}/conf/postgres/pgrepl-recovery.conf" /tmp
...
function initialize_primary() {
    ...
    if [ ! -f ${PGDATA?}/postgresql.conf ]; then
        ...
        cp "${QiNGCLOUD_DIR}/bin/postgres/setup.sql" /tmp
...
configure_archiving() {
    ...
    then
        ...
        then
            cat "${QiNGCLOUD_DIR}/conf/postgres/backrest-archive-command-local-and-s3" >> /"${PGDATA?}"/postgresql.conf
        elif [[ "${BACKREST_LOCAL_AND_GCS_STORAGE}" == "true" ]]
        then
            cat "${QiNGCLOUD_DIR}/conf/postgres/backrest-archive-command-local-and-gcs" >> /"${PGDATA?}"/postgresql.conf
        else
            cat "${QiNGCLOUD_DIR}/conf/postgres/backrest-archive-command" >> /"${PGDATA?}"/postgresql.conf
        fi
    elif [[ "${ARCHIVE_MODE}" == "on" ]] && [[ ! "${PGBACKREST}" == "true" ]]
    then
        echo_info "Setting standard archive command.."
        cat "${QiNGCLOUD_DIR}/conf/postgres/archive-command" >> /"${PGDATA?}"/postgresql.conf
    fi
...
then
    ...
    source "${QiNGCLOUD_DIR}/bin/postgres/pgbackrest.sh"
fi

configure_archiving

source "${QiNGCLOUD_DIR}/bin/postgres/custom-configs.sh"
...
source "${QiNGCLOUD_DIR}/bin/postgres/sshd.sh"
