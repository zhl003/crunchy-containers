...
source "${RADONDB_DIR}/bin/postgres-ha/common/common_lib.sh"
...
if echo "${conf_content}" | "${RADONDB_DIR}/bin/yq" x --tojson - "${bootstrap_file}" postgresql
then
    cleanup
    exit 0
fi
...
"${RADONDB_DIR}/bin/yq" d -i "${bootstrap_file}" postgresql && \
    "${RADONDB_DIR}/bin/yq" m -i "${bootstrap_file}" "${merge_file}"
