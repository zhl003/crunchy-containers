...
# Set defaults for the custom radondb-postgres-ha env vars required to bootstrap a cluster
...
build_bootstrap_config_file() {
    ...
    cat "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pghba-bootstrap.yaml" > "${pghba_file}"

    if [[ "${PGHA_BASE_BOOTSTRAP_CONFIG}" == "true" ]]
    then
        echo_info "Applying base bootstrap config to postgres-ha configuration"
        "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-bootstrap.yaml"
    ...
    then
        ...
        "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pgconf.yaml"
    else
    ...
    then
        ...
        "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pgbackrest.yaml"
        if [[ "${PGHA_PGBACKREST_LOCAL_S3_STORAGE}" == "true" ]]
        then
            ${RADONDB_DIR}/bin/yq m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pgbackrest-local-s3.yaml"
        fi
        if [[ "${PGHA_PGBACKREST_LOCAL_GCS_STORAGE}" == "true" ]]
        then
            ${RADONDB_DIR}/bin/yq m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pgbackrest-local-gcs.yaml"
        fi
    else
    ...
    if [[ "${PGHA_INIT}" == "true" ]]
    then
        "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-initdb.yaml"
        ...
        then
            ...
            if printf '10\n'${PGVERSION} | sort -VC
            then
                "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" 'bootstrap.initdb[+].waldir' "${PGHA_WALDIR}"
            else
                "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" 'bootstrap.initdb[+].xlogdir' "${PGHA_WALDIR}"
            fi
        fi
    fi
    ...
    then
        ...
        "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-standby.yaml"
    fi

    if [[ "${PGHA_SYNC_REPLICATION}" == "true" ]]
    then
        echo_info "Applying synchronous replication settings to postgres-ha configuration"
        "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-sync.yaml"
    fi
    ...
    then
      ...
      pghba_tls_file="${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pghba-tls.yaml"
      ...
      then
        ...
        "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" postgresql.authentication.replication.sslkey "${tls_replication_key_file}"
        "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" postgresql.authentication.replication.sslcert "${tls_replication_cert_file}"
        "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" postgresql.authentication.replication.sslrootcert "/pgconf/tls/ca.crt"
        # as we have the CA available, we can make the TLS mode "verify-ca" by
        # default
        "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" postgresql.authentication.replication.sslmode "verify-ca"
        # if a CRL file is provided, add this to the stanza as well
        if [[ -f "/pgconf/tls/ca.crl" ]]
        then
          "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" postgresql.authentication.replication.sslcrl "/pgconf/tls/ca.crl"
        fi

        # use an updated pg_hba.conf file that enforces certificate based
        # authentication
        pghba_tls_file="${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pghba-tls-auth.yaml"
      fi

      echo_info "Applying TLS remote connection configuration to pg_hba.conf"
      "${RADONDB_DIR}/bin/yq" m -i -a "${pghba_file}" "${pghba_tls_file}"
      echo_info "Enabling TLS in postgresql.conf"
      "${RADONDB_DIR}/bin/yq" m -i -a "${bootstrap_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pgconf-tls.yaml"

      # The CRL file may not be present, so we only want to apply if if we have
      # the CRL file present
      if [[ -f "/pgconf/tls/ca.crl" ]]
      then
        "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" bootstrap.dcs.postgresql.parameters.ssl_crl_file "/pgconf/tls/ca.crl"
      fi
    fi
    ...
    then
      ...
      "${RADONDB_DIR}/bin/yq" m -i -a "${pghba_file}" "${RADONDB_DIR}/conf/postgres-ha/postgres-ha-pghba-notls.yaml"
    fi
    ...
    then
      "${RADONDB_DIR}/bin/yq" w -i "${bootstrap_file}" bootstrap.dcs.postgresql.parameters.password_encryption "scram-sha-256"
    fi
    ...
    then
        ...
        then
            ...
            "${RADONDB_DIR}/bin/yq" w -i --style=single "${bootstrap_file}" postgresql.parameters.archive_command "false"
        fi
    fi
    ...
    "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "${pghba_file}"

    if [[ -f "/pgconf/postgres-ha.yaml" ]]
    then
        echo_info "Applying custom postgres-ha configuration file"
        "${RADONDB_DIR}/bin/yq" m -i -x "${bootstrap_file}" "/pgconf/postgres-ha.yaml"
    else
